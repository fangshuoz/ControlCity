...
from .osm_unet import OSMUNet as UNet2DConditionModel
        ...
        if self.metadata_embedding is not None:
            ...
            if cond_metadata is not None:
                ...
                if len(cond_metadata.shape) == 3:
                    ...
                    cond_metadata = self.time_proj(cond_metadata.view(-1)).view(md_bsz, self.num_metadata, num_cond, -1)
                    ...
                    md_bsz = cond_metadata.shape[0]
                    cond_metadata = self.time_proj(cond_metadata.view(-1)).view(md_bsz, self.num_metadata, -1)
                    cond_metadata = cond_metadata.to(dtype=self.dtype)
                    for i, md_embed in enumerate(self.metadata_embedding):
                        md_emb = md_embed(cond_metadata[:, i, :])
                        emb = emb + md_emb
